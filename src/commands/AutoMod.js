...
module.exports = {
    ...
    /**
     * [NEW] هل هذي الرتبة تعطي رؤية قناة وحدة على الأقل بالسيرفر (حتى لو
     * بدون أي صلاحية كتابة)؟ فحص فعلي على مستوى overwrites القنوات، مو بس
     * صلاحيات الرتبة الخام. تُستخدم لتحديد هل العضو "لسه يقدر يمارس سلوكه
     * كعضو عادي" بعد إزالة رتبة معينة منه بأمر !demote اليدوي تحديداً.
     */
    roleGrantsChannelAccess(guild, role) {
        return guild.channels.cache.some(ch =>
            ch.type !== 4 /* ChannelType.GuildCategory */ &&
            typeof ch.permissionsFor === 'function' &&
    ...
    async demoteMember(member, reason, roleToRemoveInput = null) {
        ...
        if (currentRoles.size === 0) {
            ...
            return { success: false, roleName: null, error: 'NO_DEMOTABLE_ROLES' };
        }
        ...
        if (roleToRemoveInput) {
            // [FIX] سبب فشل محدد ("العضو ما يملك هذي الرتبة أصلاً") بدل فشل
            // عام - قبل هذا كان يرجع نفس نتيجة مشاكل التسلسل الهرمي بالضبط،
            // ويضلل المشرف برسالة "Hierarchy" رغم إن السبب مختلف تماماً.
            if (!currentRoles.has(roleToRemoveInput.id)) {
                ...
                return { success: false, roleName: null, error: 'MEMBER_LACKS_ROLE', roleQueried: roleToRemoveInput.name };
            }
        ...
        if (botMember.roles.highest.position <= primaryRemovedRole.position) {
            ...
            return { success: false, roleName: null, error: 'BOT_HIERARCHY_TOO_LOW', roleQueried: primaryRemovedRole.name };
        }

        try {
            // [FIX] فقط لأمر !demote اليدوي (roleToRemoveInput محدد بالضبط):
            // لو العضو بعد إزالة هذي الرتبة المحددة لسه يملك رتبة ثانية على
            // الأقل تعطيه رؤية قناة وحدة (حتى لو بدون أي صلاحية كتابة)، ما
            // نضيف له أي رتبة بديلة إطلاقاً. قبل هذا التعديل، البوت كان
            // يضيف رتبة "آمنة" عشوائياً بكل الحالات حتى لو مالها أي داعي -
            // المشرف قصده يشيل رتبة وحدة بس، مو يسوي تخفيض شامل.
            //
            // ⚠️ هذا القيد يطبّق حصراً على المسار اليدوي. المسار التلقائي
            // (AutoMod عند تجاوز عتبة سبام/تحذيرات - roleToRemoveInput=null،
            // يشيل كل رتب العضو) يبقى بسلوكه الأصلي بالضبط زي ما هو -
            // تنزيله لرتبة بدون صلاحيات (حتى بدون رؤية قناة) مقبول ومقصود
            // هناك، ولا نلمسه إطلاقاً.
            if (roleToRemoveInput) {
                const remainingRoles = currentRoles.filter(r => r.id !== roleToRemoveInput.id);
                const hasVisibleRemainingRole = remainingRoles.some(r => this.roleGrantsChannelAccess(guild, r));

                if (hasVisibleRemainingRole) {
                    await member.roles.remove(rolesToStrip, reason);
                    console.log(`[AutoMod] Demote for ${member.user.tag}: member retains other visible role(s) - no replacement role added.`);
            ...
            return { success: false, roleName: null, error: 'INTERNAL_ERROR' };
        }
    },
/**
