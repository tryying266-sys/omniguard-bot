...
/**
 * 2. CORE LOGIC: executeUnwarn
 * [مخصصة لأمر الشات !unwarn فقط] تحذف آخر تحذير نشط وحدة - سلوك تقليدي
 * تراكمي (كل استدعاء يشيل تحذير وحدة). لا تُستخدم من الداشبورد.
 */
...
/**
 * 2.b CORE LOGIC: clearAllWarnings
 * [مخصصة لزر "Clear Warnings" بالداشبورد فقط] تمسح كل التحذيرات النشطة
 * دفعة وحدة - هذا هو المعنى الفعلي لكلمة "Clear" بالزر (تصفية كاملة،
 * مو حذف واحد). منفصلة تماماً عن executeUnwarn عشان أمر الشات !unwarn
 * يبقى بسلوكه التقليدي (تحذير وحدة بكل استدعاء) بدون أي تأثر.
 */
async function clearAllWarnings(guild, targetId, moderator, reason, dbUtils) {
    try {
        const supabase = require('../supabase/db');
        ...
        if (!count || count === 0) {
            return { success: false, error: "User has no active warnings." };
        }
        ...
        if (deleteError) {
            return { success: false, error: "Failed to remove warnings." };
        }

        await dbUtils.addInfraction(guild.id, targetId, moderator.id, 'unwarn', `Cleared ${count} warning(s) via Dashboard: ${reason}`, null);
...
module.exports = {
    ...
    executeWarn,
    executeUnwarn,
    clearAllWarnings, // [NEW] تُستخدم من apiRouter.js لزر "Clear Warnings" بالداشبورد فقط
    run
};