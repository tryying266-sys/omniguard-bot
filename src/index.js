...
// [NEW] Ticket System - panel button / select-menu "open ticket" interaction
// flow (the piece ticketsystem.js's own header comment says lives in its
// own file - it's not part of that file's scope).
const { handleTicketPanelInteraction } = require(path.join(__dirname, 'events', 'ticketPanel'));
...
// [NEW] ticketAutomation.js: Auto Close Scheduler (كل 60 ثانية) + Idle
// Watcher/Auto Respond (messageCreate) - يصدّر (client) => {...} بنفس نمط
// الثلاثة اللي فوقه بالضبط.
const ticketAutomation = require(path.join(__dirname, 'commands', 'ticketAutomation'));
// [NEW] autoRespond.js: Auto Respond Engine (messageCreate) - يستمع لكل
// رسالة ويطابقها مع قواعد auto_respond_rules المُدارة من داشبورد
// AutoRespond.html. يصدّر (client) => {...} بنفس نمط الأربعة اللي فوقه
// بالضبط.
// [NEW] autoRespond.js: Auto Respond Engine (messageCreate)
const autoRespondEngine = require(path.join(__dirname, 'commands', 'autoRespond'));

// [NEW] Command Aliases Engine - استيراد دالة تهيئة كاش الألقاب المستعارة
const { initAliasCache } = require(path.join(__dirname, 'commands', 'Aliases'));
...
client.on('interactionCreate', async (interaction) => {
    // [NEW] Ticket Panel: the "Open Ticket" button and the ephemeral
    // select-menu that follows it. Routed here separately since neither is
    // a ChatInputCommand.
    if (interaction.isButton() || interaction.isStringSelectMenu()) {
        const ticketCustomIds = ['ticket_panel_open', 'ticket_panel_select'];
        if (!ticketCustomIds.includes(interaction.customId)) return;

        try {
            await handleTicketPanelInteraction(interaction);
        } catch (error) {
            console.error('[Ticket Panel Interaction Error]:', error);
            const errorResponse = { content: '❌ An internal error occurred while processing this ticket action.', ephemeral: true };
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        // [FIX] Use ?? instead of || so that an empty string "" (no prefix) is respected
...
// [NEW] ticketAutomation.js يسجل client.once('ready', ...) داخلي خاص فيه
// (بدء السكجولر كل 60 ثانية) + client.on('messageCreate', ...) - لازم
// يُستدعى هنا قبل client.login()، نفس سبب الأربعة اللي فوقه بالضبط.
ticketAutomation(client);
// [NEW] autoRespond.js يسجل client.on('messageCreate', ...) داخلي خاص فيه
// (فحص كل رسالة ضد قواعد auto_respond_rules) - نفس مبدأ الأربعة اللي
// فوقه، لازم يُستدعى هنا قبل client.login() (listener منفصل تماماً عن
// messageCreate الرئيسي بالأعلى، ما يتعارض معه).
autoRespondEngine(client);

// [NEW] Poll Management System
